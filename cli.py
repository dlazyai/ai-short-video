...
def _bgm_type(value: str) -> str:
    ...
    if normalized == "none":
        return ""
    if normalized in {"", "random", "custom", "generated"}:
        return normalized
    raise argparse.ArgumentTypeError(
        "bgm-type must be one of: none, random, custom, generated"
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate AI Short Video videos without the WebUI.\n\n"
            "Model settings and the dlazy API key are read from config.toml."
        ),
        epilog="""
Examples:
...
""",
    ...
    audio_group.add_argument(
        ...
        type=_bgm_type,
        default=None,
        metavar="{none,random,custom,generated}",
        help=(
            "background music mode; generated produces an instrumental track "
            "through dlazy; --bgm-file implies custom when omitted "
            "(default: random)"
        ),
    )
    audio_group.add_argument(
        "--video-music-prompt",
        default=None,
        help="optional mood prompt for generated background music",
    )
    audio_group.add_argument(
        "--bgm-file",
        default=None,
    ...
    if args.video_music_prompt:
        if args.bgm_type in (None, "generated"):
            args.bgm_type = "generated"
        else:
            parser.error(
                "--video-music-prompt can only be combined with --bgm-type generated"
            )
