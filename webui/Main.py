...
# 避免依赖中的同名 app 包遮蔽 AI Short Video 自己的 app 包。
...
from app.services import cache_manager, dlazy_client, llm, video, voice, webui_task
...
st.set_page_config(
    page_title="AI Short Video",
    page_icon="🤖",
    layout="wide",
    initial_sidebar_state="auto",
    menu_items={
        "Report a bug": "https://github.com/dlazyai/ai-short-video/issues",
        "About": "# AI Short Video\nSimply provide a topic or keyword for a video, and it will "
        "automatically generate the video copy, video materials, video subtitles, "
        "and video background music before synthesizing a high-definition short "
        "video.\n\nhttps://github.com/dlazyai/ai-short-video",
    },
)


# Streamlit 1.59 会在页面右上角默认展示 Deploy、skills nudge 等平台入口。
# AI Short Video 是面向终端用户的本地工具，这些入口会造成顶部大块空白，
...
def _apply_pending_task_restore():
    ...
    video_source = params.get("video_source") or "generated"
    ...
    # voice_name 形如 "<tts-model>:<voice>"；旧任务没有前缀时回落到默认模型。
    tts_server = (
        voice_name.split(":", 1)[0]
        if ":" in voice_name
        else voice.DEFAULT_TTS_MODEL
    )
    ...
    st.session_state["generated_bgm_prompt_input"] = (
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="mpt-brand">
            <span class="mpt-brand__name">AI Short Video</span>
            <a class="mpt-brand__version"
               href="https://github.com/dlazyai/ai-short-video"
               target="_blank"
               rel="noopener noreferrer"
               aria-label="Open AI Short Video on GitHub"
...
def _render_generation_task_snapshot(task_id, task):
    ...
    for warning in task.get("warnings") or []:
        if isinstance(warning, Mapping) and warning.get("code") == "dlazy_bgm_failed":
            st.warning(
                tr("Generated BGM Fallback Warning").format(
...
DLAZY_LLM_MODELS = ["claude-sonnet-5", "qwen3.8-max", "kimi-k3"]
DLAZY_ASR_MODELS = ["fun-asr", "elevenlabs-stt"]
DLAZY_TTS_MODELS = ["qwen-tts", "doubao-tts", "elevenlabs-tts"]
DLAZY_VIDEO_MODELS = [
    "seedance-2.0-fast", "seedance-2.0", "seedance-2.5",
    "veo-3.1-fast", "kling-v3", "wan2.7",
]
DLAZY_MUSIC_MODELS = ["elevenlabs-music", "suno-music", "search_audio"]


def _dlazy_model_select(label_key, config_key, candidates, default, help_key=None):
    """Model picker bound to a `dlazy` config key.

    The candidate list is filtered through the account manifest so the dropdown
    only ever offers models this key can actually run.
    """
    try:
        options = dlazy_client.available_models(candidates)
    except Exception:
        options = candidates
    current = str(config.dlazy.get(config_key) or default)
    if current not in options:
        current = options[0] if options else default
    chosen = stable_selectbox(
        tr(label_key),
        options=options,
        default_value=current,
        key=f"dlazy_{config_key}_select",
    )
    _set_runtime_config("dlazy", config_key, chosen)
    if help_key:
...
def _render_settings_dialog():
    with st.container():
        ...
        dlazy_panel, cache_config_panel, left_config_panel = st.tabs(
            [
                tr("dlazy Settings Tab"),
        ...
        # 唯一的对外凭据：一个 dlazy key 覆盖文案、配音、字幕、素材和配乐。
        with dlazy_panel:
            st.caption(tr("One key covers script, voice-over, subtitles, footage and music."))

            key_panel, model_panel = st.columns([1, 1], gap="large")

            with key_panel:
                api_key = st.text_input(
                    tr("dlazy API Key"),
                    value=str(config.dlazy.get("api_key", "") or ""),
                    type="password",
                    key="dlazy_api_key_input",
                    help=tr("Get it at dlazy.com/dashboard/organization/api-key"),
                )
                _set_runtime_config("dlazy", "api_key", api_key)

                base_url = st.text_input(
                    tr("dlazy Base URL"),
                    value=str(config.dlazy.get("base_url", "") or ""),
                    key="dlazy_base_url_input",
                    placeholder="https://dlazy.com",
                )
                _set_runtime_config("dlazy", "base_url", base_url)

                if st.button(
                    "📡 " + tr("Check API"), key="dlazy_check", use_container_width=True
                ):
                    with st.spinner(tr("Check API") + "..."):
                        ok = dlazy_client.check_credentials()
                    st.toast(
                        tr("API Key is valid") if ok else tr("API Key is invalid"),
                        icon="✅" if ok else "❌",
                    )

            with model_panel:
                _dlazy_model_select(
                    "LLM Model", "llm_model", DLAZY_LLM_MODELS, "claude-sonnet-5",
                    "Writes the script and the search terms",
                )
                _dlazy_model_select(
                    "ASR Model", "asr_model", DLAZY_ASR_MODELS, "fun-asr",
                    "Transcribes the voice-over into subtitles",
                )
                _dlazy_model_select(
                    "Footage Model", "video_model", DLAZY_VIDEO_MODELS,
                    "seedance-2.0-fast",
                    "Generates one clip per search term; billed per clip",
                )
                _dlazy_model_select(
                    "Music Model", "music_model", DLAZY_MUSIC_MODELS,
                    "elevenlabs-music",
                    "Background music; search_audio picks a royalty-free track instead of generating one",
                )
...
def _render_video_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            # 生成式素材按所选画幅出片,不再需要按 source 修正默认值;
            # widget key 仍带 source,用户在某个来源下改过画幅会被记住。
...
def _dlazy_preview_signature(tts_model: str) -> dict:
    """试听缓存的失效凭据：换模型或换密钥都应重新合成。"""
    return {
        "provider": tts_model,
        "credential": _credential_signature(
            str(config.dlazy.get("api_key", "") or "")
        ),
        "voice": str(config.dlazy.get("tts_voice", "") or ""),
    }
...
def _render_voice_preview(params, friendly_names, selected_tts_server, voice_name):
    ...
    provider_signature = _dlazy_preview_signature(selected_tts_server)
...
def _render_background_music_settings(params):
    ...
    bgm_options = [
        ...
        (tr("Generated Background Music"), "generated"),
    ]
    ...
    if params.bgm_type == "generated":
        # 配乐同样走设置面板里那一个 dlazy key，这里只挑模型。
        _dlazy_model_select(
            "Music Model", "music_model", DLAZY_MUSIC_MODELS, "elevenlabs-music"
        )
    ...
    if params.bgm_type == "generated":
        params.video_music_prompt = st.text_input(
            tr("Music Prompt"),
            key="generated_bgm_prompt_input",
            max_chars=music_service.MAX_MUSIC_SECONDS * 4,
            help=tr("Music Prompt Help"),
        ).strip()
        if params.video_count > 1:
            st.warning(tr("Generated Music Multiple Videos Warning"))
...
def _render_audio_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            # 配音走 dlazy：这里只选模型和音色，密钥统一在设置面板里配。
            saved_tts_model = str(
                config.dlazy.get("tts_model") or voice.DEFAULT_TTS_MODEL
            )
            try:
                tts_model_options = dlazy_client.available_models(
                    list(voice.TTS_MODELS)
                )
            except Exception:
                tts_model_options = list(voice.TTS_MODELS)
            if saved_tts_model not in tts_model_options:
                saved_tts_model = tts_model_options[0]

            if tts_mode_enabled:
                selected_tts_server = stable_selectbox(
                    tr("Voiceover Service"),
                    options=tts_model_options,
                    default_value=saved_tts_model,
                    key="tts_model_select",
                )
            else:
                # 非自动配音模式不渲染 TTS 控件，但保留上次选择，切回后可以继续使用。
                selected_tts_server = saved_tts_model

            _set_runtime_config("dlazy", "tts_model", selected_tts_server)
            _set_runtime_config("ui", "tts_server", selected_tts_server)

            # 音色表直接读 dlazy manifest，dlazy 新增音色时界面自动跟上。
            filtered_voices = []
            if tts_mode_enabled:
                try:
                    voice_ids, default_voice = dlazy_client.list_voices(
                        selected_tts_server
                    )
                except Exception:
                    voice_ids, default_voice = [], ""
                if not voice_ids:
                    fallback = voice.DEFAULT_VOICES.get(selected_tts_server, "")
                    voice_ids = [fallback] if fallback else []
                    default_voice = fallback
                filtered_voices = [f"{selected_tts_server}:{v}" for v in voice_ids]

            friendly_names = {v: v.split(":", 1)[1] for v in filtered_voices}
            ...
            if tts_mode_enabled and friendly_names:
                ...
                default_choice = (
                    saved_voice_name if saved_voice_name in options else options[0]
                )
                ...
                _set_runtime_config(
                    "dlazy", "tts_voice", voice_name.split(":", 1)[-1]
                )
...
def _render_generation_controls(
    ...
    if start_button:
        ...
        if params.video_source == "generated" and not str(
            config.dlazy.get("api_key", "") or ""
        ).strip():
            _remove_active_generation_task(task_id)
            st.error(tr("dlazy API Key Required"))
            st.stop()

        if (
            params.bgm_type == "generated"
            and bgm_service.should_use_bgm(params.bgm_type, params.bgm_volume)
            and not str(config.dlazy.get("api_key", "") or "").strip()
        ):
            _remove_active_generation_task(task_id)
            st.error(tr("dlazy API Key Required"))
            st.stop()
