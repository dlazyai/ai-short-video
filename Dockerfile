...
WORKDIR /ai-short-video

# 设置/ai-short-video目录权限为777
RUN chmod 777 /ai-short-video

ENV PYTHONPATH="/ai-short-video"
...
# docker build -t ai-short-video .

# 2. Run the Docker container using the following command
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/ai-short-video/config.toml -v $(pwd)/storage:/ai-short-video/storage -p 127.0.0.1:8501:8501 ai-short-video
## For Windows:
# docker run -v ${PWD}/config.toml:/ai-short-video/config.toml -v ${PWD}/storage:/ai-short-video/storage -p 127.0.0.1:8501:8501 ai-short-video
