...
def _generate_response(prompt: str, app_config=None) -> str:
    ...
    ``{"texts": ["..."]}`` — there is no messages array, no temperature and no
    ``response_format``. Everything above this function already parses plain
    text (or JSON embedded in it) out of the reply, so this stays a drop-in
    replacement for the former multi-provider dispatch.

    The ``Error: ...`` return convention is preserved: callers such as
    ``test_connection`` and ``generate_script`` detect failures by that prefix
    rather than by exceptions.
    """
    try:
        runtime_app_config = app_config if app_config is not None else config.app
        model = str(
            runtime_app_config.get("dlazy_llm_model")
            or config.dlazy.get("llm_model")
        ...
        output = dlazy_client.run_tool(
            model,
            {"prompt": prompt, "images": [], "videos": [], "promptRefs": []},
        )
        texts = (output or {}).get("texts") or []
        content = texts[0] if texts else ""
        if not content:
            raise Exception(f"[{model}] returned an empty response")
        return _normalize_text_response(content, model)
